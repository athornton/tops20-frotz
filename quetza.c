...
/*
 * This is used only by A00028. It probably should be allocated
 ...
 */
zword A00029 (FILE *svf, FILE *stf)
{
    ...
    {
	A00199 ("This is not a saved game file!\n");
	return 0;
    }
	...
	switch (tmpl)
	{
		...
		{
		    A00199 ("Save file has two IFZS chunks!\n");
		    return fatal;
		}
		progress |= GOT_HEADER;
		...
		{
		    A00199 ("File was not saved from this story!\n");
		    return fatal;
		}
		if ((x = get_c (svf)) == EOF)			return fatal;
		pc = (zlong) x << 16;
		...
		{
		    A00199 ("File contains two stack chunks!\n");
		    break;
		}
		progress |= GOT_STACK;
		...
		if (A00035 != V6)
		{
		    ...
		    if (tmpw > STACK_SIZE)
		    {
			A00199 ("Save-file has too much stack (and I can't cope).\n");
			return fatal;
		    }
		    currlen -= 8;
		...
		for (fp = stack+STACK_SIZE, A00077 = 0;
		     currlen > 0;
		     currlen -= 8, ++A00077)
		{
		    if (currlen < 8)				return fatal;
			...
			if (zmp[tmpl] != (zbyte) x)
			{
			    A00199 ("Save-file has wrong variable number on stack (possibly wrong game version?)\n");
			    return fatal;
			}
		    }
		    ...
		    *--sp = (zword) (fp - stack - 1);	/* FP */
		    ...
		    {
			A00199 ("Save-file uses incomplete argument lists (which I can't handle)\n");
			return fatal;
		    }
		    *--sp = tmpw | i;
		...
		if (!(progress & GOT_MEMORY))	/* Don't complain if two. */
		{
			...
			if (x == 0)	/* Start run. */
			{
			    ...
			    if (currlen < 2)
			    {
				A00199 ("File contains bogus `CMem' chunk.\n");
				for (; currlen > 0; --currlen)
			...
			if (i > A00043)
			{
			    A00199 ("warning: `CMem' chunk too long!\n");
     ...
     */
    if (!(progress & GOT_HEADER))
	A00199 ("error: no valid header (`IFhd') chunk in file.\n");
    if (!(progress & GOT_STACK))
	A00199 ("error: no valid stack (`Stks') chunk in file.\n");
    if (!(progress & GOT_MEMORY))
	A00199 ("error: no valid memory (`CMem' or `UMem') chunk in file.\n");
 ...
 */
zword A00028 (FILE *svf, FILE *stf)
{
    ...
    if (!write_word (svf, A00037))			return 0;
    for (i=H_SERIAL; i<H_SERIAL+6; ++i)
	if (!write_byte (svf, zmp[i]))			return 0;
    if (!write_word (svf, A00048))			return 0;
    ...
    for (i=0, j=0, cmemlen=0; i < A00043; ++i)
    {
